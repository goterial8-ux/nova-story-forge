...
Style reset before every major planned beat:
- Keep the same POV.
- Keep the same voice and paragraph rhythm.
- Continue from the previous beat's exit.
- Do not switch into summary mode.
- Do not lose the protagonist's current emotional and physical state.
- Do not restart the story or repeat old context.
...
function getLastApprovedPartVoiceAnchor(state: ProjectState, partNumber: number): string {
  ...
  const lastLine = lastNonEmptyLine(lastApproved.draftText).slice(-300);

  return [
    `VOICE ANCHOR — last 500 characters of the previous approved part (Part ${lastApproved.partNumber} "${lastApproved.partTitle}"):`,
    `"${lastContent}"`,
    "",
    `LAST LINE OF PREVIOUS APPROVED PART: "${lastLine}"`,
    "",
    "IMPORTANT: Match this voice, tone, paragraph rhythm, and sentence length. Continue from the last line's emotional and plot state without rewriting the previous part.",
  ].join("\n");
}

function splitPlanIntoBeatBlocks(currentPartPlan: string): string[] {
  const text = String(currentPartPlan || "").trim();
  if (!text) return [];

  const blocks = text
    .split(/(?=^\s*(?:Scene|SCENE|Beat|BEAT|Step|STEP)?\s*\d+(?:[.)]|\s*[—–:-]))/m)
    .map((x) => x.trim())
...
function lastSentenceOf(text: string): string {
  ...
  return clean.slice(-160).trim();
}

function buildPlanBeatAnchorsFromPlan(currentPartPlan: string): string {
  const blocks = splitPlanIntoBeatBlocks(currentPartPlan);
  if (blocks.length === 0) return "";

  const anchors: string[] = [];
  let previousExit = "Start from the current part opening pressure.";

  for (let i = 0; i < blocks.length; i++) {
    const block = blocks[i];
    const firstLine = block.split("\n")[0].trim();
    const title = firstLine.slice(0, 130) || `Planned beat ${i + 1}`;

    anchors.push(
      [
        `PLAN BEAT ${i + 1} ANCHOR — ${title}`,
        `Previous beat exit: "${previousExit.slice(0, 180)}"`,
        "Before writing this beat, reset: same POV, same recap voice, same paragraph rhythm, same protagonist state, no summary mode.",
      ].join("\n"),
    );

    previousExit = lastSentenceOf(block) || previousExit;
  }
  ...
  return [
    ...
    "=== PLAN BEAT ANCHORS ===",
    anchors.join("\n\n"),
    "Use these as internal anchors while expanding the Current Part Plan. Do not output the anchors or labels.",
  ].join("\n");
}
